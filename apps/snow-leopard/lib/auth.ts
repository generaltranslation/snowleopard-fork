       ...
       if (!user.email) {
        ...
        try {
          ...
          const { data, error } = await resend.emails.send({
            ...
            html: `<p>${t('Welcome! Please click the link below to verify your email address:')}</p><p><a href="${url}">${t('Verify Email')}</a></p><p>${t('If the link doesn\'t work, copy and paste this URL into your browser: {url}', { url })}</p>`,
          });
